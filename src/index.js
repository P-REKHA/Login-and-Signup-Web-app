...
app.post('/login', async (req, res) => {
	const check = await dbcol.findOne({ username: req.body.username });
	if (check == null) {
		res.status(201).render('login', { info: 'Invalid user' });
	} else if (check.password != req.body.password) {
		res.status(201).render('login', { info: 'Incorrect password' });
	} else if (check.password === req.body.password) {
